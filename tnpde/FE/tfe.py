...
class FENET():
    def __init__(self,bounds,nx):
        ...
        self.gp_even = [1.0/6,2.0/3]
        self.gp_odd = [1.0/3,5.0/6]
        self.Node()
        self.Unit()
    ...
    def Node(self):#生成网格点(M + 1)*(N + 1)
        ...
        for i in range(self.nx[0] + 1):
            for j in range(self.nx[1] + 1):
                ...
                m = m + 1
    def phi(self,X,order):
        ind00 = (X[:,0] >= -1);ind01 = (X[:,0] >= 0);ind02 = (X[:,0] >= 1)
        ind10 = (X[:,1] >= -1);ind11 = (X[:,1] >= 0);ind12 = (X[:,1] >= 1)
        ind20 = (X[:,0] + X[:,1] >= -1);ind21 = (X[:,0] + X[:,1] >= 0);ind22 = (X[:,0] + X[:,1] >= 1)
        if order == [0,0]:
            return (ind01*~ind02*ind11*~ind12*~ind22).astype('float32')*(1 - X[:,0] - X[:,1]) + \
                    (ind01*~ind02*ind10*~ind11*ind21).astype('float32')*(1 - X[:,0]) + \
                    (ind01*~ind02*ind10*~ind11*~ind21).astype('float32')*(X[:,1] + 1) + \
                    (ind00*~ind01*ind10*~ind11*ind20).astype('float32')*(1 + X[:,0] + X[:,1]) + \
                    (ind00*~ind01*ind11*~ind12*~ind21).astype('float32')*(1 + X[:,0]) + \
                    (ind00*~ind01*ind11*~ind12*ind21).astype('float32')*(1 - X[:,1])
        if order == [1,0]:
            return (ind01*~ind02*ind11*~ind12*~ind22).astype('float32')*(- 1.0) + \
                    (ind01*~ind02*ind10*~ind11*ind21).astype('float32')*(- 1.0) + \
                    (ind00*~ind01*ind10*~ind11*ind20).astype('float32')*1.0 + \
                    (ind00*~ind01*ind11*~ind12*~ind21).astype('float32')*1.0 
    ...
    def Unit(self):
        self.Units_size = 2*self.nx[0]*self.nx[1]
        self.Units_Nodes = np.zeros([self.Units_size,3],np.int)#每个单元有3个点，记录这3个点的整体编号
        self.Units_Int_Points = np.zeros([self.Units_size,#划分成2*M*N个小区域，每个区域有3个高斯积分点
                                          3,self.dim])
        m = 0
        for i in range(self.nx[0]):
            for j in range(self.nx[1]):
                self.Units_Nodes[2*m,0] = i*(self.nx[1] + 1) + j
                self.Units_Nodes[2*m,1] = i*(self.nx[1] + 1) + j + 1
                self.Units_Nodes[2*m,2] = (i + 1)*(self.nx[1] + 1) + j
                self.Units_Int_Points[2*m,0,0] = self.bounds[0,0] + (i + self.gp_even[0])*self.hx[0]
                self.Units_Int_Points[2*m,0,1] = self.bounds[1,0] + (j + self.gp_even[0])*self.hx[1]
                self.Units_Int_Points[2*m,1,0] = self.bounds[0,0] + (i + self.gp_even[0])*self.hx[0]
                self.Units_Int_Points[2*m,1,1] = self.bounds[1,0] + (j + self.gp_even[1])*self.hx[1]
                self.Units_Int_Points[2*m,2,0] = self.bounds[0,0] + (i + self.gp_even[1])*self.hx[0]
                self.Units_Int_Points[2*m,2,1] = self.bounds[1,0] + (j + self.gp_even[0])*self.hx[1]
                #-----------------
                self.Units_Nodes[2*m + 1,0] = (i + 1)*(self.nx[1] + 1) + j
                self.Units_Nodes[2*m + 1,1] = i*(self.nx[1] + 1) + j + 1
                self.Units_Nodes[2*m + 1,2] = (i + 1)*(self.nx[1] + 1) + j + 1
                self.Units_Int_Points[2*m + 1,0,0] = self.bounds[0,0] + (i + self.gp_odd[1])*self.hx[0]
                self.Units_Int_Points[2*m + 1,0,1] = self.bounds[1,0] + (j + self.gp_odd[0])*self.hx[1]
                self.Units_Int_Points[2*m + 1,1,0] = self.bounds[0,0] + (i + self.gp_odd[0])*self.hx[0]
                self.Units_Int_Points[2*m + 1,1,1] = self.bounds[1,0] + (j + self.gp_odd[1])*self.hx[1]
                self.Units_Int_Points[2*m + 1,2,0] = self.bounds[0,0] + (i + self.gp_odd[1])*self.hx[0]
                self.Units_Int_Points[2*m + 1,2,1] = self.bounds[1,0] + (j + self.gp_odd[1])*self.hx[1]
                m = m + 1
    ...
    def Int_basic_basic(self,i,j,u_ind):#表示第i,j个基函数的梯度的乘积，以及在第u_ind个区域的积分
        X = self.Units_Int_Points[u_ind,:,:]#[3,2]
        ...
        return ((basic0*basic1).sum(1)).mean()*self.hx[0]*self.hx[1]*0.5
    def Int_F_basic(self,i,u_ind,prob):#第i个基函数与右端项乘积，在第u_ind个单元积分
        X = self.Units_Int_Points[u_ind,:,:]
        return (FF(prob,X)*self.basic(X,[0,0],i)).mean()*self.hx[0]*self.hx[1]*0.5
    ...
    def matrix(self,beta):
        ...
        for m in range(self.Units_size):#self.Units_size = 2*M*N，第m个区域单元的4个积分点
            for k in range(3):
                ind0 = self.Units_Nodes[m,k]#self.Units_Nodes = [M*N,3],第m个区域中第k个网格点，第k个基函数编号
                for l in range(3):
...
#print(fenet.Units_size,fenet.Units_Nodes.shape,fenet.Nodes.shape,fenet.Nodes_size,fenet.Units_Int_Points.shape)
nx_te = [80,80]
