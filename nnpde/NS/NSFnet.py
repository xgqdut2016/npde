...
def train_yp(netu,netv,netp,inset,bdset,penalty_in,penalty_bd,optim,epoch,error_history,optimtype):
    ...
    for it in range(epoch):
        ...
        netv.zero_grad()
        netp.zero_grad()
        parameters = itertools.chain(netu.parameters(),netv.parameters(),netp.parameters())
        loss = Loss_yp(netu,netv,netp,inset,bdset,penalty_in,penalty_bd)
        bd3 = bdset.loss3
        bd3.backward()
        grads = [_.grad for _ in parameters]
        x = torch.cat([abs(pa.reshape(-1)) for pa in grads])
        fenzi = max(x)
        #----
        netu.zero_grad()
        netv.zero_grad()
        ...
        parameters = itertools.chain(netu.parameters(),netv.parameters(),netp.parameters())
        loss = Loss_yp(netu,netv,netp,inset,bdset,penalty_in,penalty_bd)
        loss_div = inset.loss_div
        loss_div.backward()
        grads = [_.grad for _ in parameters]
        x = torch.cat([abs(pa.reshape(-1)) for pa in grads])
        fenmu = penalty_in[2]*x.mean()
        penalty_in[2] = fenzi/fenmu
        #----
        netu.zero_grad()
        netv.zero_grad()
        netp.zero_grad()
        ...
        grads = [_.grad for _ in parameters]
        x = torch.cat([abs(pa.reshape(-1)) for pa in grads])
        fenmu = penalty_in[0]*x.mean()
        penalty_in[0] = fenzi/fenmu
        #----
        netu.zero_grad()
        netv.zero_grad()
        netp.zero_grad()
        ...
        grads = [_.grad for _ in parameters]
        x = torch.cat([abs(pa.reshape(-1)) for pa in grads])
        fenmu = penalty_in[1]*x.mean()
        penalty_in[1] = fenzi/fenmu
        #----
        netu.zero_grad()
        netv.zero_grad()
        netp.zero_grad()
        ...
        grads = [_.grad for _ in parameters]
        x = torch.cat([abs(pa.reshape(-1)) for pa in grads])
        fenmu = penalty_bd[0]*x.mean()
        penalty_bd[0] = fenzi/fenmu
        #----
        netu.zero_grad()
        netv.zero_grad()
        netp.zero_grad()
        ...
        grads = [_.grad for _ in parameters]
        x = torch.cat([abs(pa.reshape(-1)) for pa in grads])
        fenmu = penalty_bd[1]*x.mean()
        penalty_bd[1] = fenzi/fenmu
        '''#----
        netu.zero_grad()
        netv.zero_grad()
        netp.zero_grad()
        ...
        grads = [_.grad for _ in parameters]
        x = torch.cat([abs(pa.reshape(-1)) for pa in grads])
        fenmu = penalty_bd[2]*x.mean()
        penalty_bd[2] = fenzi/fenmu
        '''#----
        netu.zero_grad()
        netv.zero_grad()
        netp.zero_grad()
        ...
        grads = [_.grad for _ in parameters]
        x = torch.cat([abs(pa.reshape(-1)) for pa in grads])
        fenmu = penalty_bd[3]*x.mean()
        penalty_bd[3] = fenzi/fenmu
        #----
        netu.zero_grad()
        netv.zero_grad()
        netp.zero_grad()
        ...
        grads = [_.grad for _ in parameters]
        x = torch.cat([abs(pa.reshape(-1)) for pa in grads])
        fenmu = penalty_bd[4]*x.mean()
        penalty_bd[4] = fenzi/fenmu
        #----------------------------
...
fname1 = "unsf-NSlay%dvar.pt"%(wid)
fname2 = "vnsf-NSlay%dvar.pt"%(wid)
fname3 = "pnsf-NSlay%dvar.pt"%(wid)
